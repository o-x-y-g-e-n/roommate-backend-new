...
const stripe = require('stripe')(
  'sk_test_51IlJLyHixsK8VUAYxdjHLuclpi1Cb6aMxYDk5LVqmmiUbuS1V4YX4FDW1P1iX7WljWEiMP0yfzQUoJzlEse83ota007X9hFBi5'
);
...
userRouter.post('/choreCheck', async function (req, res) {
  console.log(req.body);
  let update = await choreModel.findOneAndUpdate(
    { Item: req.body.Item },
...
userRouter.post('/addrequest', async function (req, res) {
  ...
  async function sendMail() {
    let transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: 'mireshsonkamble@gmail.com',
        pass: 'Ytrewq@4321',
      },
    });
...
userRouter.post('/getcomplaints', async function (req, res) {
  ...
  requestModel.find(
    { roomKey: roomkey },
    await function (err, foundRequests) {
      if (err) {
        console.log(err);
      } else {
...
userRouter.post('/updateChores', async function (req, res) {
  ...
  for (let i = 0; i < numChores; ++i) {
    ...
    let update = await choreModel.findOneAndUpdate(
      { Item: req.body.chores[i].Item },
      { Name: req.body.roomies[i % req.body.roomies.length] }
    );
    console.log(update);
  }
...
userRouter.post('/getRooms', async function (req, res) {
  ...
  user.find({ email: email }, async function (err, foundTenants) {
    ...
    for (let i = 0; i < allrooms.length; i++) {
      ...
      user.find({ roomKey: currkey }, function (err, foundMates) {
        ...
        for (let n = 0; n < foundMates.length; ++n) {
          ...
          room[2].push([
            foundMates[n].firstName,
            foundMates[n].rentPaid,
            foundMates[n].email,
          ]);
        }
      });
